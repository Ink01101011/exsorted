import { combSort } from '../src/sorted/parallel/comb/combSort';
...
describe('combSort', () => {
  describe('numbers (ascending, default)', () => {
    it('sorts an unsorted array', () => {
      expect(combSort([5, 3, 8, 1, 2])).toEqual([1, 2, 3, 5, 8]);
    });
    ...
    it('handles a single-element array', () => {
      expect(combSort([42])).toEqual([42]);
    });

    it('handles an array with duplicate values', () => {
      expect(combSort([3, 1, 4, 1, 5, 9, 2, 6, 5, 3])).toEqual([1, 1, 2, 3, 3, 4, 5, 5, 6, 9]);
    });

    it('handles an array where all elements are equal', () => {
      expect(combSort([7, 7, 7, 7])).toEqual([7, 7, 7, 7]);
    });

    it('handles negative numbers', () => {
      expect(combSort([-3, -1, -4, -2])).toEqual([-4, -3, -2, -1]);
    });

    it('handles mixed negative and positive numbers', () => {
      expect(combSort([3, -2, 0, -1, 2])).toEqual([-2, -1, 0, 2, 3]);
    });
  });

  describe('numbers (descending, custom comparator)', () => {
    it('sorts in descending order', () => {
      expect(combSort([5, 3, 8, 1, 2], (a, b) => b - a)).toEqual([8, 5, 3, 2, 1]);
    });
  });

  describe('strings (alphabetical, default)', () => {
    it('sorts strings alphabetically', () => {
      expect(combSort(['banana', 'apple', 'cherry'])).toEqual(['apple', 'banana', 'cherry']);
    });
  });
  ...
  describe('objects (without custom comparator)', () => {
    it('sorts deterministically with the default comparator', () => {
      ...
      expect(combSort(input)).toEqual([{ id: 1 }, { id: 2 }]);
    });
  });
  ...
  describe('parallel exports', () => {
    it('re-exports combSort from the parallel index', () => {
      expect(parallelExports.combSort).toBe(combSort);
    });
  });

  describe('sorted index exports', () => {
    it('re-exports combSort from the sorted index', () => {
      expect(sortedExports.combSort).toBe(combSort);
    });
  });
});
