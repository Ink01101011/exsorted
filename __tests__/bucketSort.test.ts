...
describe('bucketSort', () => {
  it('sorts numbers in ascending order', () => {
    expect(bucketSort([5, 3, 8, 1, 2])).toEqual([1, 2, 3, 5, 8]);
  });

  it('handles duplicates and negative numbers', () => {
    expect(bucketSort([3, -1, 3, 0, -1, 2])).toEqual([-1, -1, 0, 2, 3, 3]);
  });
  ...
  it('supports objects with a key selector', () => {
    ...
    const result = bucketSort(input, (item) => item.score);
  ...
  it('throws when sorting objects without keySelector', () => {
    ...
    expect(() => bucketSort(input)).toThrow(ERROR_MESSAGES.KEY_SELECTOR_REQUIRED);
  });

  it('throws when key is not an integer', () => {
    expect(() => bucketSort([1.1, 2, 3])).toThrow(ERROR_MESSAGES.KEY_NOT_INTEGER);
    expect(() => bucketSort([{ score: 1 }, { score: 2.5 }], (item) => item.score)).toThrow(
